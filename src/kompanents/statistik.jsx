...
export default function Statistics() {
  ...
  const text = statisticsText[i18n.language]; // tilga qarab matnlarni oladi
  ...
  return (
    <section ref={ref} className="bg-orange-50 py-24 px-6">
      ...
      <div className="text-center mb-14"
           style={{opacity: active ? 1 : 0, transform: active ? "none" : "translateY(16px)", transition:"all 0.6s ease"}}>
        ...
        <h2 className="text-4xl font-black text-slate-800 leading-tight">{text.header.title}</h2>
        <p className="mt-3 text-slate-400 text-sm max-w-xs mx-auto">{text.header.desc}</p>
      </div>
      ...
      {/* Bottom banner */}
      <div className="max-w-5xl mx-auto mt-8 bg-orange-600 rounded-3xl px-8 py-6 flex flex-wrap justify-around items-center gap-6"
           style={{opacity: active?1:0, transition:"opacity 0.7s 0.6s ease", boxShadow:"0 8px 32px rgba(59,130,246,0.25)"}}>
